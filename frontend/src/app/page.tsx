...
export default function DDRGenerator() {
  ...
  const validateFile = (file: File): string | null => {
    ...
    if (file.size > MAX_SIZE) {
      return `File size exceeds 25MB limit. File size: ${formatFileSize(file.size)}`;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('  Size:', inspection.size, 'bytes (' + formatFileSize(inspection.size) + ')');
    ...
    console.log('  Size:', thermal.size, 'bytes (' + formatFileSize(thermal.size) + ')');
                ...
                <div className="text-center">
                  ...
                  {inspection && (
                    <p className="text-green-400 text-sm mt-1">
                      ✓ {formatFileSize(inspection.size)}
                    </p>
                  )}
                  {!inspection && (
          ...
          <div className="mt-12 space-y-8 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-lg p-6 border border-slate-700 hover:border-slate-600 transition-all transform hover:scale-105">
                <p className="text-slate-400 text-xs mb-2">📄 Inspection Size</p>
                <p className="text-2xl font-bold text-white">{inspection ? formatFileSize(inspection.size) : 'N/A'}</p>
              </div>
              <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-lg p-6 border border-slate-700 hover:border-slate-600 transition-all transform hover:scale-105">
                <p className="text-slate-400 text-xs mb-2">🔥 Thermal Size</p>
                <p className="text-2xl font-bold text-white">{thermal ? formatFileSize(thermal.size) : 'N/A'}</p>
              </div>
              <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-lg p-6 border border-slate-700 hover:border-slate-600 transition-all transform hover:scale-105">
                <p className="text-slate-400 text-xs mb-2">📊 Total Obs</p>
                <p className="text-2xl font-bold text-white">{report.total_observations}</p>
              </div>
              <div className="bg-gradient-to-br from-blue-900 to-slate-900 rounded-lg p-6 border border-blue-700 hover:border-blue-600 transition-all transform hover:scale-105">
                <p className="text-blue-300 text-xs mb-2">🔄 Merged Obs</p>
                <p className="text-2xl font-bold text-blue-400">{report.merged_observations}</p>
              </div>
              <div className="bg-gradient-to-br from-orange-900 to-slate-900 rounded-lg p-6 border border-orange-700 hover:border-orange-600 transition-all transform hover:scale-105">
                <p className="text-orange-300 text-xs mb-2">⚠️ Conflicts Found</p>
                <p className="text-2xl font-bold text-orange-400">{report.conflicts_found}</p>
              </div>
            </div>
              ...
              {report.observations && report.observations.length > 0 && (
                <div>
                  <p className="text-slate-400 text-sm mb-3 font-semibold">
                    🔍 Detailed Observations ({report.observations.length})
                  </p>
                  <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                    {report.observations.map((obs: any, idx: number) => {
                      const isMajor = obs.severity === 'MAJOR' || obs.severity?.toUpperCase() === 'HIGH' || obs.severity_hint === 'major';
                      const isMinor = obs.severity === 'MINOR' || obs.severity?.toUpperCase() === 'LOW' || obs.severity_hint === 'minor';
                      
                      return (
                      <div key={idx} className={`p-5 rounded-xl border backdrop-blur-sm transition-all transform hover:-translate-y-1 ${
                        isMajor ? 'bg-red-900/10 border-red-500/30 hover:border-red-500/50 shadow-[0_0_15px_rgba(239,68,68,0.1)]' : 
                        isMinor ? 'bg-yellow-900/10 border-yellow-500/30 hover:border-yellow-500/50 shadow-[0_0_15px_rgba(234,179,8,0.1)]' : 
                        'bg-blue-900/10 border-blue-500/30 hover:border-blue-500/50 shadow-[0_0_15px_rgba(59,130,246,0.1)]'
                      }`}>
                        <div className="flex justify-between items-start mb-3">
                           <div>
                             <h4 className="font-bold text-white text-lg flex items-center gap-2">
                               📍 <span>{obs.area || 'Property'} Issue</span>
                               <span className={`px-2 py-0.5 ml-2 text-xs font-bold rounded border ${
                                 isMajor ? 'bg-red-500/20 text-red-400 border-red-500/30' : 
                                 isMinor ? 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30' : 
                                 'bg-blue-500/20 text-blue-400 border-blue-500/30'
                               }`}>
                                 {(obs.severity || obs.severity_hint || 'UNKNOWN').toUpperCase()} SEVERITY
                               </span>
                             </h4>
                           </div>
                        </div>
                        
                        <div className="text-slate-300 leading-relaxed text-sm mt-3 bg-white/5 p-3 rounded-lg border border-white/5 space-y-1">
                            <p><span className="text-slate-400 font-semibold">Inspection:</span> {obs.inspection_issue && obs.inspection_issue !== 'Not Available' ? obs.inspection_issue : 'No specific issue identified'}</p>
                            <p><span className="text-slate-400 font-semibold">Thermal:</span> {obs.thermal_issue && obs.thermal_issue !== 'Not Available' ? obs.thermal_issue : 'No thermal anomalies detected'}</p>
                            {obs.description && obs.description.trim() !== "" && (
                                <p className="pt-2 mt-2 border-t border-white/10 text-xs italic opacity-70">{obs.description}</p>
                            )}
                        </div>
                        
                        {/* Display Extracted Images if available */}
                        {obs.images && Array.isArray(obs.images) && obs.images.length > 0 ? (
                          <div className="mt-4 grid grid-cols-2 gap-4">
                            {obs.images.map((img: any, imgIdx: number) => (
                              <div key={imgIdx} className="bg-black/30 rounded-lg p-2 border border-slate-700">
                                <p className="text-xs text-slate-400 mb-2 capitalize font-semibold">{img.type} Image</p>
                                {img.data_uri === "Image Not Available" ? (
                                  <div className="h-32 flex flex-col items-center justify-center bg-slate-800/50 rounded text-slate-500 text-xs text-center border-2 border-dashed border-slate-700 p-2">
                                    <span>No relevant {img.type} image available.</span>
                                  </div>
                                ) : (
                                  <img 
                                    src={img.data_uri} 
                                    alt={`${img.type} observation`} 
                          ...
                          <div className="mt-4 p-3 bg-black/20 rounded border border-slate-800 text-xs text-slate-500 text-center italic">
                            No relevant inspection or thermal images available for this observation.
                          </div>
                        )}
                      </div>
                    )})}
