                    ...
                    {report.observations.map((obs: any, idx: number) => {
                      ...
                      <div key={idx} className={`p-5 rounded-xl border backdrop-blur-sm transition-all transform hover:-translate-y-1 ${
                        ...
                        <div className="flex justify-between items-start mb-3">
                           <div>
                             <h4 className="font-bold text-white text-lg">{obs.issue || 'Unknown Issue'}</h4>
                             <p className="text-sm font-medium text-slate-400 flex items-center gap-1 mt-1">
                               📍 <span>{obs.area || 'Unknown Area'}</span>
                             </p>
                           </div>
                           <span className={`px-3 py-1 text-xs font-bold rounded-full border ${
                             ...
                             {(obs.severity || obs.severity_hint || 'UNKNOWN').toUpperCase()}
                           </span>
                        </div>
                        <p className="text-slate-300 leading-relaxed text-sm mt-3 bg-white/5 p-3 rounded-lg border border-white/5">{obs.description || 'No description available.'}</p>
                            ...
                            {obs.images.map((img: any, imgIdx: number) => (
                              <div key={imgIdx} className="bg-black/30 rounded-lg p-2 border border-slate-700">
                                ...
                                {img.data_uri === "Image Not Available" ? (
                                  <div className="h-32 flex items-center justify-center bg-slate-800/50 rounded text-slate-500 text-xs text-center border-2 border-dashed border-slate-700">
                                    Image Not Available
                                  </div>
                                ) : (
                                  <img 
                        ...
                        {obs.source && (
                          <div className="mt-4 flex gap-2 justify-end">
                            <span className="text-xs bg-slate-800/80 text-slate-400 px-2 py-1 rounded border border-slate-700">Source: {obs.source}</span>
                            {obs.page && <span className="text-xs bg-slate-800/80 text-slate-400 px-2 py-1 rounded border border-slate-700">Page: {obs.page}</span>}
                          </div>
                        )}
                      </div>
                    )})}
